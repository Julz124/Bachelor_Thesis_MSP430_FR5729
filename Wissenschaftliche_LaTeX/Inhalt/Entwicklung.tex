...
Dieser Abschnitt beschreibt das \glqq{}Rückgrat\grqq{} des Observer-Moduls. Dazu zählen die Zustandsmaschine sowie verschiedene unterstützende Funktionen und Datenstrukturen.
...
Das in \Kapitel{ZustaendeUndFunktionen} erläuterte Konzept der zustandsbasierten Verarbeitung eingehender Befehle basiert auf den in \Listing{datenstruktur_zustaende} dargestellten \Fachbegriff{Ein Funktionsprototyp legt Signatur und Rückgabetyp einer Funktion fest, ohne deren Implementierung bereitzustellen.}{Funktionsprototypen}[Funktionsprototyp].

\vspace{0.5cm}
\begin{lstlisting} [style=modernC, caption={Datenstruktur der Funktionszeiger f\"ur die Zustandsmaschine}, label={lst:datenstruktur_zustaende}]
	/*
	* State Machine
	*/
...
Die angegebenen Funktionsprototypen repräsentieren die jeweiligen Zustände, in denen sich das System zu einem bestimmten Zeitpunkt befindet. Abhängig vom aktuellen Verarbeitungsschritt referenziert der zentrale Funktionszeiger \Code{state\_ptr} eine dieser Funktionen, die anschließend innerhalb der Timer-Interrupt-Service-Routine (ISR) ausgeführt wird.
...
\begin{itemize}
	...
	\newpage
    \item In \textbf{\Code{state\_0}} ist der Timeout-Timer für die Eingabe von Zeichen über die Benutzerkonsole aktiv (\Listing{state_0}).
    ...
    \item \textbf{\Code{state\_1}} überprüft den Eingabepuffer auf das Vorliegen eines gültigen Befehls (\Listing{state_1}).
	...
	\newpage
    \item \textbf{\Code{state\_2}} setzt das Observer-Modul in einen definierten Anfangszustand zurück (\Listing{state_2}).
...
Das in \Kapitel{Init} vorgestellte Dictionary, in dem Befehle ihren zugehörigen Funktionszeigern zugewiesen werden, implementiert \Listing{Observer_func_dict}.
