...
Diese Vorteile gelten gleicherma{\ss}en f\"ur die gegen\"uberliegende Seite der Schnittstelle: Alle g\"angigen Betriebssysteme wie Windows, Linux oder macOS stellen standardm\"aßig Treiber f\"ur die asynchrone serielle Daten\"ubertragung bereit. Unter Windows erfolgt dies beispielsweise \"uber sogenannte \NeuerBegriff{COM-Ports}, w\"ahrend unter Linux Schnittstellen wie \NeuerBegriff{/dev/ttySx} oder \NeuerBegriff{/dev/ttyUSBx} verwendet werden.
...
\newpage
Weitere technische Spezifikationen sind in \Tabelle{uart_features} zusammengefasst. Eine minimale Konfiguration der Schnittstelle auf den UART-Betrieb wird in \ref{eUSCI_Konfiguration} detailliert beschrieben.
...
Die vorangegangenen Kapitel haben die Kommunikationsschnittstelle des MSP430FR5729 im UART-Betriebsmodus detailliert beleuchtet. Im Kern des eUSCI-Moduls steht die serielle Kommunikation mit Peripherieger\"aten oder Systemen. Dazu werden synchrone Protokolle wie SPI und I$^{2}$C und asynchrone Protokolle wie UART bereitgestellt.

Im Rahmen dieser Arbeit kristallisierte sich in \ref{Einordnung_Schnittstellen} heraus, dass sich das asynchrone Kommunikationsprotokoll UART am besten f\"ur die Kommunikation zwischen dem MSP430FR5729 und einem PC-System geeignet ist. Dabei wurden die Schnittstellen unter den Gesichtspunkten der Implementierung, Robustheit, Interruptf\"ahigkeit und Kompartibilit\"at betrachtet.

Wesentliche Aspekte des UART-Betriebs ist die zuvor angesprochene asynchrone Kommunikation, welche eine ungest\"orte, wechselseitige Kommunikation beider Seiten, \"uber eine Baudraten-Synchronisation, erm\"oglicht. Wichtige Merkmale hierzu sind die Formate der Daten\"ubertragung (\zB acht Datenbits, LSB-first, ein SP) und die der Fehlererkennung (\zB Framing-, Parity-, Overrun-Error).

Die Automatisierte Baudraten-Erkennung schafft, bei verkn\"upfung mehrerer Kommunikationspartner \"uber die gleiche Leitung, eine hohe flexibilit\"at, erh\"oht jedoch die komplexit\"at dabei erheblich. Da allerdings zu diesem Zeitpunkt nicht mehrere Verbindungen unterschiedlicher Baudraten \"uber eine Schnittstelle erfolgen, ist auch diese Funktion nicht vonn\"oten.

Zentrale Schritte zur Konfiguration des eUSCI-Moduls f\"ur den UART-Betrieb ist das einleiten des eUSCI-Reset-Modus, das Setzen und R\"ucksetzen von notwendigen Steuerbits, die Feinabstimmung der gew\"unschten Baudrate \"uber die Prescaler und das aufheben des Reset-Zustandes.

Ein detailliertes Blockdiagramm des eUSCI\_A-Moduls, das f\"ur den UART-Betrieb konfiguriert ist, ist in \Abbildung{BlockDiagramm_eUSCI_A_UART} dargestellt.
