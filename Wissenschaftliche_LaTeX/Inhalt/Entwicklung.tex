...
Aufbauend auf den in den vorangegangenen Kapiteln -- \ref{TIMER&ISR} und \ref{eUSCI} -- vermittelten theoretischen Grundlagen zu Timern, Interrupt Service Routinen (ISR) und dem Enhanced Universal Serial Communication Interface des Mikrocontrollers MSP430FR5729, widmet sich dieses Kapitel der konkreten Entwicklung und praktischen Umsetzung der interruptgesteuerten Benutzerschnittstelle.

Zun\"achst erfolgt die systematische Konzeption des Observer-Moduls, das als zentrale Instanz zur Erfassung von Benutzereingaben sowie zur Steuerung der Systemausgaben fungiert. Dieses Modul stellt die Grundlage f\"ur die Interaktion zwischen Benutzer und Mikrocontroller dar und \"ubernimmt die Koordination integrierter Steuerbefehle.

Im weiteren Verlauf wird die interruptgesteuerte Kommunikation \"uber die UART-Schnittstelle konzeptioniert. Hierbei liegt der Fokus auf zuverl\"assigkeit und robustheit sowie deren sicherer Ausgabe unter Echtzeitbedingungen.
...
Die Konzeption und Integration von Breakpoints verfolgt dabei das Ziel, gezielt in den Programmablauf eingreifen zu k\"onnen, ohne den Systemzustand zu beeintr\"achtigen. Insbesondere die Funktionen \Code{read\_mem} und \Code{write\_mem} profitieren hiervon: Wird ein Breakpoint erreicht, k\"onnen diese Befehle kontrolliert und st\"orungsfrei ausgef\"uhrt werden, da das Hauptprogramm w\"ahrenddessen in einen definierten Haltemodus versetzt wird. Auf diese Weise l\"sst sich eine konsistente Speicherzugriffslogik realisieren, die auch w\"ahrend kritischer Zust\"ande wie Laufzeitunterbrechungen zuverl\"assig funktioniert.\AI

\section{Konzeprionierung Observer-Modul}
\label{ObserverModulKonzept}

Die zentrale Einheit zur Verarbeitung eingehender Befehle und zur Steuerung s\"amtlicher Funktionen stellt das \glqq Observer-Modul\grqq dar. Es vereint die interruptbasierte Abarbeitung sowie das Senden von Daten mit dem Empfang externer Steuerbefehle. Die in \ref{TIMER&ISR} und \ref{eUSCI} erl\"auterten Technologien des MSP430FR5729 bilden die technische Grundlage.

Solange kein Befehl \"uber die Eingabetaste eines verbundenen PCs best\"atigt wurde, verbleibt das Modul im Ruhezustand. Bereits ab dem ersten \Fachbegriff[Zeichenkombination aus Buchstaben (lateinisches Alphabet) und Ziffern (0–9)]{alphanumerischen} Zeichen wird ein \glqq Time-Out\grqq-Timer aktiviert. Erfolgt keine abschlie{\ss}ende Eingabe innerhalb eines definierten Zeitintervalls, wird die Eingabe zur\"uckgesetzt und ein \glqq Time-Out-Error\grqq generiert. Nach erfolgreicher Best\"atigung wird der Timer deaktiviert und die Befehlsinterpretation eingeleitet. Das Ergebnis der Verarbeitung wird \"uber UART zur\"uckgegeben. Etwaige Fehler im Ablauf werden mithilfe eines Fehlervektors priorisiert, gespeichert und entsprechend ausgegeben. Das Observer-Modul arbeitet statusorientiert und verarbeitet Ereignisse gem\"a{\ss} ihres Auftretens.
...
Die Verarbeitung von Ereignissen wie Befehlsinterpretation, Fehlerausgabe und Eingabe-Time-Out wird \"uber eine gemeinsame ISR realisiert. Eine daf\"ur typische Periodendauer betr\"agt 10\,ms. Kapitel \ref{TIMER&ISR} beschreibt die verf\"ugbaren Timer des MSP430FR5729. Aufgrund der dort beschriebenen Merkmale eignet sich insbesondere ein Timer vom Typ B f\"ur die Realisierung.
...
Ein besonderer Vorteil ergibt sich durch den Einsatz eines \Fachbegriff[Datenstruktur, die Schl\"ussel-Wert-Paare speichert und schnellen Zugriff auf Werte \"uber ihre eindeutigen Schl\"ussel erlaubt.]{Dictionary}s, das Befehlen eindeutig Funktionszeiger zuordnet.
